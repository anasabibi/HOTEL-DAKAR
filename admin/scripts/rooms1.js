...
function add_room() {
    ...
    xhr.onload = function() {
        ...
        if (this.responseText == 1) {
            alert('success', 'Nouvelle chambre ajoutée!');
            add_room_form.reset();
            get_all_rooms();
        } else {
...
function add_image() {
    ...
    xhr.onload = function() {
        if (this.responseText == 'inv_img') {
            alert('error', 'Seules les images JPG et PNG sont autorisées !', 'image-alert');
        } else if (this.responseText == 'inv_size') {
            alert('error', 'L\'image doit être inférieure à 2 Mo !', 'image-alert');
        } else if (this.responseText == 'upd_failed') {
            alert('error', 'Échec du téléchargement de l\'image. Serveur en panne!', 'image-alert');
        } else {
...
function rem_image(img_id,room_id)
{
    ...
    xhr.onload = function() {
        if (this.responseText == 1) {
            ...
            alert('error', 'Échec de la suppression de l\'image!', 'image-alert');
        }
    }
    xhr.send(data);
}
...
function thumb_image(img_id,room_id)
{
    ...
    xhr.onload = function() {
        if (this.responseText == 1) {
            alert('success', 'Miniature de l\'image changée!', 'image-alert');
            room_images(room_id, document.querySelector("#room-images .modal-title").innerText);
        } 
        else {
            alert('error', 'Échec de la mise à jour de la miniature!', 'image-alert');
        }
    }
    xhr.send(data);
}
...
function remove_room(room_id) 
{
    if (confirm("Êtes-vous sûr de vouloir supprimer cette chambre?")) {
        ...
        xhr.onload = function () {
            if (this.responseText == 1) {
                alert('success','Chambre supprimée avec succès!');
                get_all_rooms();
            } else {
                alert('error', 'Échec de la suppression de la chambre!');
            }
        }
